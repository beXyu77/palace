...
class ResolutionPanel extends ConsumerWidget {
  const ResolutionPanel({super.key});
      ...
      children: [
        ...
        if (run.lastStatDelta.isEmpty)
          ...
          Wrap(
            ...
            children: run.lastStatDelta.entries
                .map((e) => _pill('${_statLabel(e.key)} ${_deltaText(e.value)}'))
                .toList(),
          ),
        ...
        if (run.lastFactionDelta.isEmpty)
          ...
          Wrap(
            ...
            children: run.lastFactionDelta.entries
                .map((e) => _pill('${_factionLabel(e.key)} ${_deltaText(e.value)}'))
                .toList(),
          ),
        ...
        SizedBox(
          ...
          child: FilledButton(
            onPressed: () {
              if (hasEnding) {
                Navigator.of(context).pop('toEnding'); // 交给外层处理跳转
                return;
              }

              if (isMonthEnd) {
                Navigator.of(context).pop('toMonthEnd');
              } else {
                ctrl.nextDayOrMonthEnd();
                ctrl.drawTodayEvent();
                Navigator.of(context).pop(); // 关弹窗，回到 loop 弹窗/页面
              }
            },
            child: Text(hasEnding ? '进入结局' : (isMonthEnd ? '进入月末考评' : '进入下一天')),
          ),
        ),
      ],
    );
